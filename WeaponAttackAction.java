...
public class WeaponAttackAction extends AbstractAttackAction implements
    ...
    private static ToHitData toHit(IGame game, int attackerId,
        ...
        ghostTargetModifier(game, target, ae, wtype, usesAmmo, atype, toHit);

        //Space ECM
        spaceEcm(game, target, ae, toHit);

        // Aeros may suffer from criticals
        aeroSufferFromCritical(game, ae, weapon, wtype, toHit);
        ...
        // weapon to-hit modifier
        weaponToHitModifier(target, ae, weapon, wtype, toHit);

        // ammo to-hit modifier
        ammoToHitModifier(game, te, usesAmmo, atype, toHit);
        ...
        heatSeekingMissiles(game, target, ae, te, bHeatSeeking, toHit);
        ...
        addTargetingComputer(aimingAt, aimingMode, ae, wtype, usesAmmo, atype, toHit);

        // Change hit table for elevation differences inside building.
        changeHitTableForElevationDifference(target, toHit, los, aElev, tElev);

        // Aeros in atmosphere can hit above and below
        aerosCanHitAboveAndBelow(game, target, ae, toHit, aElev, tElev);
        ...
        // underwater(legs)
        changeHitTableForPartialCover(game, te, toHit, targEl, los, targHex, underWater);
        ...
        instanceOfAero(game, target, ae, toHit);
        ...
        rmvOldTargetMov(game, aimingAt, aimingMode, exchangeSwarmTarget, oldTarget, ae, te, atype, isECMAffected,
				inSameBuilding, toSubtract, toHit, eistatus, proneMod);

        // okay!
        return toHit;
    }

	private static void changeHitTableForPartialCover(IGame game, Entity te, ToHitData toHit, int targEl,
			LosEffects los, IHex targHex, boolean underWater) {
	...
	private static void instanceOfAero(IGame game, Targetable target, final Entity ae, ToHitData toHit) {
		if (target instanceof Aero) {
	...
	private static void weaponToHitModifier(Targetable target, final Entity ae, final Mounted weapon,
			final WeaponType wtype, ToHitData toHit) {
		if (wtype instanceof VariableSpeedPulseLaserWeapon) {
            int nRange = ae.getPosition().distance(target.getPosition());
            int[] nRanges = wtype.getRanges(weapon);
            int modifier = wtype.getToHitModifier();
	...
	private static void ammoToHitModifier(IGame game, Entity te, final boolean usesAmmo, final AmmoType atype,
			ToHitData toHit) {
		if ((te != null)
                && ((te.getMovementMode() == IEntityMovementMode.VTOL)
	...
	private static void heatSeekingMissiles(IGame game, Targetable target, final Entity ae, Entity te,
			boolean bHeatSeeking, ToHitData toHit) {
		if (bHeatSeeking) {
            if (te == null) {
	...
	private static void addTargetingComputer(int aimingAt, int aimingMode, final Entity ae, final WeaponType wtype,
			final boolean usesAmmo, final AmmoType atype, ToHitData toHit) {
	...
	private static void changeHitTableForElevationDifference(Targetable target, ToHitData toHit, LosEffects los,
			int aElev, int tElev) {
	...
	private static void aerosCanHitAboveAndBelow(IGame game, Targetable target, final Entity ae, ToHitData toHit,
			int aElev, int tElev) {
	...
	private static void ghostTargetModifier(IGame game, Targetable target, final Entity ae, final WeaponType wtype,
			final boolean usesAmmo, final AmmoType atype, ToHitData toHit) {
	...
	private static void spaceEcm(IGame game, Targetable target, final Entity ae, ToHitData toHit) {
	...
	private static void aeroSufferFromCritical(IGame game, final Entity ae, final Mounted weapon,
			final WeaponType wtype, ToHitData toHit) {
		if (ae instanceof Aero) {
            Aero aero = (Aero) ae;
	...
	private static void windMods(IGame game, final WeaponType wtype, ToHitData toHit) {
		if (!game.getBoard().inSpace()) {
	...
	private static void rmvOldTargetMov(IGame game, int aimingAt, int aimingMode, boolean exchangeSwarmTarget,
			Entity oldTarget, final Entity ae, Entity te, final AmmoType atype, boolean isECMAffected,
			boolean inSameBuilding, int toSubtract, ToHitData toHit, int eistatus, ToHitData proneMod) {
		int targEl;
		int distance;
		IHex targHex;
		if (exchangeSwarmTarget) {
